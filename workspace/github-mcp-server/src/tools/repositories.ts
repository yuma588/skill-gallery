...
import { formatRepository, formatRepositoryList, formatResponse, formatSearchResults, ResponseFormat } from "../services/formatters.js";
import { GitHubRepository, GitHubSearchResult } from "../types.js";
...
type SearchRepositoriesInput = z.infer<typeof SearchRepositoriesSchema>;
...
type ListRepositoriesInput = z.infer<typeof ListRepositoriesSchema>;
...
Error Handling:
  ...
  - Returns "No repositories found matching '<query>'" if search returns empty`,
    ...
    async (params: SearchRepositoriesInput) => {
      try {
        ...
        const data = await githubClient.request<GitHubSearchResult<GitHubRepository>>(
          `/search/repositories`,
          "GET",
          undefined,
          queryParams
...
Error Handling:
  ...
  - Returns "Error: Permission denied" for private repos without access (403)`,
    ...
    async (params: ListRepositoriesInput) => {
      try {
        const pagination = githubClient.validatePagination(params.per_page, params.page);
        const repos = await githubClient.request<GitHubRepository[]>(
          `/users/${params.owner}/repos`,
          "GET",
          undefined,
          {
            type: params.type,
...
Error Handling:
  - Returns "Error: Resource not found" if repository doesn't exist (404)`,
    ...
    async (params: GetRepositoryInput) => {
      try {
        const repo = await githubClient.request<GitHubRepository>(
        ...
        if (params.response_format === ResponseFormat.JSON) {
          (response as any).structuredContent = repo;
        }
