...
Error Handling:
  ...
  - Returns "Error: Permission denied" for private repos without access (403)`,
        ...
        try {
            ...
            const prs = await githubClient.request(`/repos/${params.owner}/${params.repo}/pulls`, "GET", undefined, queryParams);
            if (!prs || prs.length === 0) {
                return {
                    content: [{
                            type: "text",
...
Error Handling:
  - Returns "Error: Resource not found" if PR doesn't exist (404)`,
        ...
        try {
            const pr = await githubClient.request(`/repos/${params.owner}/${params.repo}/pulls/${params.pull_number}`);
            const textContent = params.response_format === ResponseFormat.MARKDOWN
...
Error Handling:
  ...
  - Returns "Error: Validation failed" for invalid branches or input (422)`,
        ...
        try {
            const pr = await githubClient.request(`/repos/${params.owner}/${params.repo}/pulls`, "POST", {
                title: params.title,
                head: params.head,
                base: params.base,
                body: params.body,
    ...
    // Merge Pull Request
    server.registerTool("github_merge_pull_request", {
...
Error Handling:
  ...
  - Returns "Error: Resource not found" if PR doesn't exist (404)`,
        ...
        try {
            const result = await githubClient.request(`/repos/${params.owner}/${params.repo}/pulls/${params.pull_number}/merge`, "PUT", {
