...
Error Handling:
  ...
  - Returns "Error: Permission denied" for private repos without access (403)`,
        ...
        try {
            ...
            const commits = await githubClient.request(`/repos/${params.owner}/${params.repo}/commits`, "GET", undefined, queryParams);
...
Error Handling:
  - Returns "Error: Resource not found" if commit doesn't exist (404)`,
        ...
        try {
            const commit = await githubClient.request(`/repos/${params.owner}/${params.repo}/commits/${params.ref}`);
            const textContent = params.response_format === ResponseFormat.MARKDOWN
                ? formatCommit(commit)
            ...
            if (params.response_format === ResponseFormat.JSON) {
                response.structuredContent = commit;
            }
            return response;
        }
        catch (error) {
