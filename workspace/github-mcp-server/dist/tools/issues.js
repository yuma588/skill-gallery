...
Error Handling:
  ...
  - Returns "Error: Permission denied" for private repos without access (403)`,
        ...
        try {
            ...
            const issues = await githubClient.request(`/repos/${params.owner}/${params.repo}/issues`, "GET", undefined, queryParams);
...
Error Handling:
  - Returns "Error: Resource not found" if issue doesn't exist (404)`,
        ...
        try {
            const issue = await githubClient.request(`/repos/${params.owner}/${params.repo}/issues/${params.issue_number}`);
            const textContent = params.response_format === ResponseFormat.MARKDOWN
                ? formatIssue(issue)
            ...
            if (params.response_format === ResponseFormat.JSON) {
                response.structuredContent = issue;
            }
            return response;
        }
        catch (error) {
...
Error Handling:
  ...
  - Returns "Error: Validation failed" for invalid input (422)`,
        ...
        try {
            const issue = await githubClient.request(`/repos/${params.owner}/${params.repo}/issues`, "POST", {
                title: params.title,
                body: params.body,
...
Error Handling:
  ...
  - Returns "Error: Resource not found" if issue doesn't exist (404)`,
        ...
        try {
            const issue = await githubClient.request(`/repos/${params.owner}/${params.repo}/issues/${params.issue_number}`, "PATCH", {
                title: params.title,
                body: params.body,
                state: params.state,
                labels: params.labels,
