...
def draw_heart(draw, center_x, center_y, size, color, outline_color=None, outline_width=2):
    ...
    points = [
        (center_x, y2),  # 底部顶点
        (x1, center_y - size // 4),  # 左圆心
        (center_x - size // 4, y1),  # 左上顶点
        (center_x, y1 - size // 6),  # 中上顶点
        (center_x + size // 4, y1),  # 右上顶点
        (x2, center_y - size // 4),  # 右圆心
    ]
...
# 创建 GIF 构建器（Slack emoji 推荐 128x128）
...
for i in range(num_frames):
    # 创建渐变背景
    frame = Image.new('RGB', (128, 128), (255, 245, 250))  # 浅粉色背景
    ...
    # 模拟心跳：快速收缩两次，然后缓慢舒张
    if t < 0.15:
        scale = 1.0 + 0.15 * (t / 0.15)  # 第一次收缩
    elif t < 0.25:
        scale = 1.15 - 0.1 * ((t - 0.15) / 0.10)  # 第一次舒张
    elif t < 0.40:
        scale = 1.05 + 0.15 * ((t - 0.25) / 0.15)  # 第二次收缩
    else:
        scale = 1.2 - 0.2 * ((t - 0.40) / 0.60)  # 缓慢回到原位
