        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (hasTabDelimiters || hasPipes || isDenseNumbers || multipleSpaces) {
                ...
                if (currentTable && currentTable.length > 1) {
                    ...
                    console.log(`   预览: ${JSON.stringify(currentTable[0])}\n`);
                }
                currentTable = null;
            }
        }
...
async function main() {
    ...
    // 创建提取器实例
    const extractor = new PDFTableExtractorV2(pdfPath, outputDir);
